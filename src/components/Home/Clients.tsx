...
export default function Clients() {
  ...
  const testimonials = en.clients.testimonials // from en.json
  ...
  return (
    <section className="py-12 text-primary">
      <div className="section-container text-center">
        <h2 className="heading-primary mb-3">{en.clients.sectionTitle}</h2>
        <p className="text-secondary mb-10 max-w-2xl mx-auto text-base sm:text-lg">
          {en.clients.description}
        </p>
        ...
        <div className="relative">
            ...
            {testimonials.map((t, i) => (
              <SwiperSlide key={i}>
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-center px-4 sm:px-6 lg:px-0">
                  ...
                  <div className={`lg:col-span-2 ${isRTL ? "text-right" : "text-left"} text-center lg:text-inherit mt-4 lg:mt-0`}>
          ...
          {/* Desktop buttons */}
          <div className={`hidden lg:flex absolute bottom-4 gap-3 z-10 ${isRTL ? "left-4" : "right-4"}`}>
            <button
              ref={prevRef}
          ...
          <button
            ref={prevRef}
            className={`lg:hidden absolute top-1/3 ${isRTL ? "right-2" : "left-2"} w-10 h-10 sm:w-12 sm:h-12 bg-white rounded-full flex items-center justify-center text-[#4B2615] shadow-lg disabled:opacity-40 hover:bg-gray-100 z-10`}
            disabled={isBeginning}
          >
          ...
          <button
            ref={nextRef}
            className={`lg:hidden absolute top-1/3 ${isRTL ? "left-2" : "right-2"} w-10 h-10 sm:w-12 sm:h-12 bg-white rounded-full flex items-center justify-center text-[#4B2615] shadow-lg disabled:opacity-40 hover:bg-gray-100 z-10`}
            disabled={isEnd}
          >
